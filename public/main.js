...
window.addEventListener("DOMContentLoaded",async()=>{
    try{
        const allInfo=await axios.get("http://localhost:4000/get-expense")
        console.log(allInfo)
...
async function deleteExpense(key){  
    await axios.delete(`http://localhost:4000/delete-expense/${key}`)  
...
async function addExpense(e){
    try{
       ...
       const allInfo=await axios.post("http://localhost:4000/add-expense",obj)
       console.log(allInfo)
